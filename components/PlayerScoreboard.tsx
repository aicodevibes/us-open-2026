...
export function PlayerScoreboard({
  ...
  return (
    <section className={`grid grid-cols-1 lg:grid-cols-2 gap-6 ${playoffComplete ? 'opacity-50 grayscale transition-all duration-1000' : ''}`}>
      {/* Top 10 Players */}
      <Card className="bg-white border-2 border-[#06051e]/10 rounded-xl shadow-sm overflow-hidden flex flex-col">
        <CardHeader className="p-4 bg-[#06051e] text-white shrink-0">
        ...
        <CardContent className="p-0 overflow-y-auto max-h-[400px]">
          <Table>
            <TableHeader className="bg-[#f8f9fa] sticky top-0 z-10 shadow-sm">
              <TableRow className="hover:bg-transparent border-b border-[#06051e]/10">
                <TableHead className="w-12 text-center text-[#06051e] font-bold text-xs uppercase">Pos</TableHead>
                <TableHead className="text-[#06051e] font-bold text-xs uppercase">Player</TableHead>
                <TableHead className="text-[#06051e] font-bold text-xs uppercase">Drafted By</TableHead>
                <TableHead className="text-right pr-4 text-[#06051e] font-bold text-xs uppercase">Total</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {top10Players.map(p => (
                <TableRow key={p.playerName} className="hover:bg-[#06051e]/5 border-b border-[#06051e]/5">
                  <TableCell className="text-center font-bold text-[#06051e]">{p.rank}</TableCell>
                  <TableCell className="font-medium text-[#05041a]">
                    {p.playerName}
                    {p.isCut && <Badge variant="destructive" className="ml-2 text-[8px] uppercase">Cut</Badge>}
                  </TableCell>
                  <TableCell>
                    {draftedBy.has(p.playerName) ? (
                      <Badge className="bg-[#05041a] text-[#ffba00] hover:bg-[#05041a] text-[10px]">
                  ...
                  <TableCell className="text-right font-bold text-[#06051e] pr-4">{formatScore(p.total)}</TableCell>
                </TableRow>
              ))}
      ...
      <Card className="bg-white border-2 border-[#06051e]/10 rounded-xl shadow-sm overflow-hidden flex flex-col">
        <CardHeader className="p-4 bg-[#05041a] text-white shrink-0">
          <CardTitle className="text-sm uppercase tracking-widest flex items-center gap-2">
